 ...
 * Executes reviews by spawning Claude Code CLI in terminal
 */
...
const LOG_CATEGORY = 'ClaudeTerminal';
...
export class ClaudeTerminalExecutor extends BaseReviewExecutor {
  private terminalSessionId: string | null = null;

  async isAvailable(): Promise<{ available: boolean; error?: string }> {
    const logger = getLogger();
    logger.debug(LOG_CATEGORY, 'Terminal executor always available');
  ...
  async execute(
    ...
    logger.info(LOG_CATEGORY, 'Starting terminal review execution', {
    ...
    try {
      ...
      options.onStatusChange?.('Starting terminal review...');
      ...
      const sessionId = terminalManager.createSession({
        ...
        organization: '',
        project: '',
        label: `AI Review - ${context.guid.substring(0, 8)}`,
        workingDir: context.workingDir,
        contextPath: context.contextPath,
        outputPath: context.outputPath,
        prompt,
      ...
      logger.error(LOG_CATEGORY, 'Terminal review execution failed', {
        error: error.message,
        stack: error.stack,
      ...
      return { comments: [], error: error.message || 'Terminal review failed' };
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkFile = () => {
        ...
        if (elapsed > TERMINAL_TIMEOUT_MS) {
          logger.error(LOG_CATEGORY, 'Terminal review timed out', { elapsedMs: elapsed, timeoutMs: TERMINAL_TIMEOUT_MS });
          reject(new Error('Terminal review timed out after 30 minutes'));
          return;
        }
        ...
        if (elapsedSeconds > 0 && elapsedSeconds % 30 === 0 && elapsed - lastLogTime < 2000) {
          onStatusChange?.(`Waiting for terminal review... (${Math.floor(elapsedSeconds / 60)}m ${elapsedSeconds % 60}s)`);
        }
  ...
  cancel(): void {
    const logger = getLogger();
    logger.info(LOG_CATEGORY, 'Cancelling terminal review', { sessionId: this.terminalSessionId });
