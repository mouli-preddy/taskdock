 ...
 * Executes reviews by spawning Claude Code CLI in background (no terminal UI)
 */
...
const LOG_CATEGORY = 'ClaudeHeadless';
...
export class ClaudeHeadlessExecutor extends BaseReviewExecutor {
  ...
  async execute(
    ...
    logger.info(LOG_CATEGORY, 'Starting headless review execution', {
    ...
    try {
      ...
      // 4. Spawn Claude CLI in background
      logger.info(LOG_CATEGORY, 'Spawning Claude CLI', {
        workingDir: context.workingDir,
        promptFile: promptFilePath,
      });

      await this.runClaude(promptFilePath, context.workingDir, abortController.signal);
      ...
      logger.error(LOG_CATEGORY, 'Headless review execution failed', {
        error: error.message,
        stack: error.stack,
      ...
      return { comments: [], error: error.message || 'Headless review failed' };
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.childProcess.stdout?.on('data', (data) => {
        ...
        if (stdout.length % 1000 < 100) {
          logger.debug(LOG_CATEGORY, 'Claude stdout', { length: stdout.length });
        }
      });

      this.childProcess.stderr?.on('data', (data) => {
        stderr += data.toString();
        logger.debug(LOG_CATEGORY, 'Claude stderr', { data: data.toString() });
      });

      this.childProcess.on('error', (error) => {
        logger.error(LOG_CATEGORY, 'Claude process error', { error: error.message });
      ...
      this.childProcess.on('close', (code) => {
        logger.info(LOG_CATEGORY, 'Claude process closed', { code, stdoutLength: stdout.length, stderrLength: stderr.length });
        signal.removeEventListener('abort', abortHandler);
        this.childProcess = null;
        ...
        if (code === 0 || signal.aborted) {
          ...
          reject(new Error(`Claude process exited with code ${code}: ${stderr || 'Unknown error'}`));
        }
      });
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkFile = () => {
        ...
        if (elapsed > HEADLESS_TIMEOUT_MS) {
          logger.error(LOG_CATEGORY, 'Headless review timed out', { elapsedMs: elapsed, timeoutMs: HEADLESS_TIMEOUT_MS });
          reject(new Error('Headless review timed out after 30 minutes'));
          return;
        }
        ...
        if (elapsedSeconds > 0 && elapsedSeconds % 30 === 0 && elapsed - lastLogTime < 2000) {
          onStatusChange?.(`Running AI review... (${Math.floor(elapsedSeconds / 60)}m ${elapsedSeconds % 60}s)`);
        }
  ...
  cancel(): void {
    const logger = getLogger();
    logger.info(LOG_CATEGORY, 'Cancelling headless review');
